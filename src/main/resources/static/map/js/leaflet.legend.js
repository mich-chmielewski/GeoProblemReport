    ...
    L.Control.Legend = L.Control.extend({
        ...
        _buildLegendItems: function (legendContainer, legend) {
            ...
            if (legend.inactive) {
                ...
                if (L.Util.isArray(legend.layers)) {
                    for (var i = 0, len = legend.layers.length; i < len; i++) {
                        map.removeLayer(legend.layers[i]);
                        searchLayer.removeLayer(legend.layers[i]);
                    }
                } else {
                    map.removeLayer(legend.layers);
                    //searchLayer.removeLayer(legend.layers);
                }
            }
